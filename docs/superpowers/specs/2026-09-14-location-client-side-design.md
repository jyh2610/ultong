...
## 규제 관련 추가 논점 (2026-09-19, 최종 리뷰에서 발견)

이 설계의 핵심 주장은 "서버가 정밀 좌표를 받지 않는다"이며, 이는 구현/최종 리뷰에서 좌표 흐름을 전부 추적해 확인됐다. 다만 별개의 논점이 하나 남는다: `/places/search`에 실리는 `ldongRegnCd`/`ldongSignguCd` 요청은 로그인한 사용자의 인증된 요청(`Authorization: Bearer ...`)으로 전송되므로, 서버 로그 상으로는 "특정 사용자 + GPS로 도출한 시/군/구 + 타임스탬프"가 함께 남는다. 이는 기존의 수동 지역 검색 필터와 형태는 동일하지만("정밀 좌표를 아예 안 보낸다"는 단순한 주장과 달리) "신원과 연결된, GPS에서 유도된 대략적 위치"라는 점에서 법적으로는 미묘하게 다른 층위의 논점이다. 규제 판단(위치정보사업자 신고 여부) 시 이 구분을 팀/자문 검토에 함께 전달할 것.
